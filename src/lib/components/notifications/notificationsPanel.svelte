...
<Sheet.Root>
  <Sheet.Trigger asChild let:builder>
    <Button builders={[builder]} variant="ghost" class="relative">
      ...
      {#if notifications.length > 0}
        <div
          class="absolute top-1 right-2 bg-destructive rounded-full h-2 w-2 flex items-center justify-center"
        ></div>
      {/if}
    </Button>
  </Sheet.Trigger>
  <Sheet.Content side="right" class="flex flex-col gap-2">
    <Sheet.Header>
      ...
      <Sheet.Description>See past notifications here.</Sheet.Description>
    </Sheet.Header>
    ...
    <Sheet.Footer>
      <Sheet.Close asChild let:builder>
        {#if notifications.length > 0}
          <Button builders={[builder]} type="submit" on:click={markAllAsSeen}>
